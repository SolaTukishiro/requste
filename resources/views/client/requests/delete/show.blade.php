<x-app-layout>
    <x-slot name="header">
        <h2 class="request-index__page-title">削除依頼一覧</h2>
    </x-slot>
    <section class="request-index">
        <div class="request-index__container">
            <div class="request-index__grid">
                @forelse($requests as $request)
                    <a class="request-index__card" href="{{ route('client.requests.deleted.detail', $request->id) }}">
                ...
                @empty
                    <div class="request-index__empty">
                        <p class="request-index__empty-title">削除した依頼はまだありません</p>
                    </div>
                @endforelse
            </div>
        </div>
